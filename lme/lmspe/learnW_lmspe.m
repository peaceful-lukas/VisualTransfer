function W = learnW_lmspe(DS, W, U, param)
...
function appr_max_dW = computeApproximateMaxGradient(X, W, U, triplets, tripl_col, param)
    ...
    D = arrayfun(@(n) sum(exp(alpha*X(:, triplets(n, 1))'*W'*U(:, numPrototypes*(triplets(n, tripl_col)-1)+1:numPrototypes*triplets(n, tripl_col)))), 1:numTriplets);

    N = arrayfun(@(n) sum(repmat(exp(alpha*X(:, triplets(n, 1))'*W'*U(:, numPrototypes*(triplets(n, tripl_col)-1)+1:numPrototypes*triplets(n, tripl_col))), lowDim, 1).*U(:, numPrototypes*(triplets(n, tripl_col)-1)+1:numPrototypes*triplets(n, tripl_col)), 2), 1:numTriplets, 'UniformOutput', false);
    N = cat(2, N{:});
