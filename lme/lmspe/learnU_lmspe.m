function U = learnU_lmspe(DS, W, U, param)
...
function dU = computeGradient(DS, W, U, cTriplets, spTriplets, param)
    ...
    c_dU = zeros(lowDim, numPrototypes*numClasses);
    if( num_cTriplets > 0 )
        for n=1:num_cTriplets
            expUc = exp(alpha*X(:, cTriplets(n, 1))'*W'*U(:, numPrototypes*(cTriplets(n, 3)-1)+1:numPrototypes*cTriplets(n, 3)));
            Dc = sum(expUc);
            dUc = repmat(expUc, lowDim, 1).*repmat((W*X(:, cTriplets(n, 1))), 1, numPrototypes)/Dc;

            expUyi = exp(alpha*X(:, cTriplets(n, 1))'*W'*U(:, numPrototypes*(cTriplets(n, 2)-1)+1:numPrototypes*cTriplets(n, 2)));
            Dyi = sum(expUyi);
            dUyi = repmat(expUyi, lowDim, 1).*repmat((W*X(:, cTriplets(n, 1))), 1, numPrototypes)/Dyi; 

            trip_dU = zeros(lowDim, numPrototypes*numClasses);
            trip_dU(:, numPrototypes*(cTriplets(n, 3)-1)+1:numPrototypes*cTriplets(n, 3)) = dUc;
            trip_dU(:, numPrototypes*(cTriplets(n, 2)-1)+1:numPrototypes*cTriplets(n, 2)) = -dUyi;
    ...
    sp_dU = zeros(lowDim, numPrototypes*numClasses);
    if( num_spTriplets > 0 )
...
function loss = getSampleLoss(DS, W, U, param)
    ...
    cErr = 0;
    if( num_cTriplets > 0 )
        incorrs = arrayfun(@(n) max(X(:, cTriplets(n, 1))'*W'*U(:, numPrototypes*(cTriplets(n, 3)-1)+1:numPrototypes*cTriplets(n, 3))), 1:num_cTriplets);
        corrs = arrayfun(@(n) max(X(:, cTriplets(n, 1))'*W'*U(:, numPrototypes*(cTriplets(n, 2)-1)+1:numPrototypes*cTriplets(n, 2))), 1:num_cTriplets);
